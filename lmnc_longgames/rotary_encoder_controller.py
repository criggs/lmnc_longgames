...
class RotaryEncoderController:
    def __init__(self, 
        ...
        self.rotary_encoder.when_rotated_clockwise = self.build_callback(ROTATED_CW, ROTARY_PUSH)
        self.rotary_encoder.when_rotated_counter_clockwise = self.build_callback(ROTATED_CCW, ROTARY_PUSH)
        
        self.rotary_push = Button(rotary_push_button_pin)
        self.rotary_push.when_released = self.build_callback(BUTTON_RELEASED, ROTARY_PUSH)
